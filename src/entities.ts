...
//this file contain the logic of our players
...
export function makePlayer(k: KaboomCtx, posX: number, posY: number) {
  const player = k.make([
    k.sprite("assets", { anim: "kirbIdle" }), // setting kirbIdle as default
  ...
  player.onCollide("enemy", async (enemy: GameObj) => {
    ...
    player.hurt();
    // making the player flash/blink
    await k.tween(
      player.opacity,
      0,
      0.05,
      (val) => (player.opacity = val),
      k.easings.linear // what is the rate change? we set as linear
    );
    await k.tween(
      player.opacity,
      1,
  ...
  inhaleZone.onUpdate(() => {
    if (player.direction === "left") {
      ...
      inhaleEffect.flipX = true; // if it is the left side then we flip it
      return;
    }
    inhaleZone.pos = k.vec2(14, 8);
    inhaleEffect.pos = k.vec2(player.pos.x + 60, player.pos.y + 0);
